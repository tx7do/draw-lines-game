...
public class GameManager : MonoBehaviour
{
    ...
    [Range(0, 1)] public float gridCellTopBackgroundAlpha = 0.5f; //The grid cell top background alpha.
    [Range(-20, 20)] public float gridCellZPosition = 0; //The grid cell z-position.
    [Range(-50, 50)] public float gridLineZPosition = -2; //The grid line z-position.
    [Range(0, 10)] public float gridLineWidthFactor = 0.5f; //The grid line width factor.

    /// <summary>
    /// The cell content scale factor.
    /// </summary>
    [Range(0.1f, 1)] public float cellContentScaleFactor = 0.6f;

    /// <summary>
    /// The cell content z-position.
    /// </summary>
    [Range(-20, 20)] public float cellContentZPosition = -5;
    ...
    /// </summary>
    [Range(-50, 50)] public float draggingElementZPosition = 0;

    /// <summary>
    /// The dragging element scale factor.
    /// </summary>
    [Range(0, 10)] public float draggingElementScaleFactor = 1;
    ...
    private void Release(Line line)
    {
        ...
        var ps = draggingElement.transform.Find("ColorsEffect").GetComponent<ParticleSystem>();
        var em = ps.emission;
        em.enabled = false;
        ...
        if (tempCollider2D != null)
        {
            ...
            if (tempCollider2D.tag == "GridCell")
            {
                ...
                if (clickType == ClickType.Began)
                {
                    ...
                    var emission = draggingElement.transform.Find("ColorsEffect").GetComponent<ParticleSystem>()
                        .emission;
    ...
    private void GridCellClickBegan()
    {
        ...
        {
            Debug.Log("Current grid cell of index " + currentGridCell.index +
                      " is Ignored [Reason : Empty,Not Currently Used]");
            return;
        }
        ...
        else if (!currentGridCell.isEmpty)
        {
            //If the grid is not currently used and it's not empty
            ...
            Debug.Log("New GridCell of Index " + currentGridCell.index + " added to the Line Path of index " +
                      currentLine.index);
    ...
    private void GridCellClickMoved()
    {
        ...
        {
            Debug.Log("Current grid cell of index " + currentGridCell.index +
                      " is Ignored [Reason : Not Adjacent Of Previous GridCell " + previousGridCell.index);
            return;
        }
        ...
        {
            Debug.Log("Current grid cell of index " + currentGridCell.index +
                      " is Ignored [Reason : Not the wanted Traget]");
            return; //skip next
        }
        ...
        if (!currentGridCell.isEmpty)
        {
            ...
            {
                Debug.Log("Two GridCells connected [GridCell " +
                          (gridLines[currentGridCell.gridLineIndex].GetFirstPathElement()) + " with GridCell " +
                          (gridLines[currentGridCell.gridLineIndex].GetLastPathElement()) + "]");
                ...
                for (int i = 0; i < currentLine.path.Count; i++)
                {
                    ...
                    if (i == 0 || i == currentLine.path.Count - 1)
                    {
                        ...
                        GameObjectUtil.FindChildByTag(gridCell.transform, "GridCellContent")
                                .GetComponent<SpriteRenderer>().sprite =
                            currentLevel.dotsPairs[gridCell.elementPairIndex].connectSprite;
                    }
    ...
    private void SettingUpPairs()
    {
        ...
        for (int i = 0; i < currentLevel.dotsPairs.Count; i++)
        {
            ...
            numberColor = new Color(1 - elementsPair.color.r, 1 - elementsPair.color.g, 1 - elementsPair.color.b,
                1); //opposite color
            ...
            cellContentScale =
                (Mathf.Max(gridCellScale.x, gridCellScale.y) / Mathf.Min(gridCellScale.x, gridCellScale.y)) *
                cellContentScaleFactor;
            ...
            ///Create Grid Line
            CreateGridLine(cellContentScale * gridLineWidthFactor, elementsPair.lineColor,
                "Line " + elementsPair.firstDot.index + "-" + elementsPair.secondDot.index, i);
        }

        Color tempColor = Mission.wantedMission.missionColor;
        tempColor.a = draggingElementAlpha;

        CreateDraggingElement(tempColor,
            new Vector3(cellContentScale * draggingElementScaleFactor, cellContentScale * draggingElementScaleFactor,
                cellContentScale));
    }
    ...
    public void NextLevel()
    {
        if (LevelsTable.currentLevelID >= 1 && LevelsTable.currentLevelID < LevelsTable.tableLevels.Count)
        {
            ...
            DataManager.MissionData currentMissionData =
                DataManager.FindMissionDataById(Mission.wantedMission.ID,
                    DataManager.filterdMissionsData); //Get the current mission
            DataManager.LevelData
                currentLevelData =
                    currentMissionData.FindLevelDataById(TableLevel.wantedLevel.ID); ///Get the current level
            if (currentLevelData.ID + 1 < currentMissionData.levelsData.Count)
            {
                DataManager.LevelData nextLevelData =
                    currentMissionData.FindLevelDataById(TableLevel.wantedLevel.ID + 1); ///Get the next level
    ...
    private void CheckLevelComplete()
    {
        ...
        if (isLevelComplete)
        {
            ...
            try
            {
                ...
                DataManager.MissionData currentMissionData =
                    DataManager.FindMissionDataById(Mission.wantedMission.ID, DataManager.filterdMissionsData);
                DataManager.LevelData currentLevelData =
                    currentMissionData.FindLevelDataById(TableLevel.wantedLevel.ID);
                currentLevelData.starsLevel =
                    StarsRating.GetLevelStarsRating(Timer.timeInSeconds, GameManager.movements, gridCells.Length);
                if (currentLevelData.ID + 1 <= currentMissionData.levelsData.Count)
                {
                    ///Unlock the next level
                    DataManager.LevelData nextLevelData =
                        currentMissionData.FindLevelDataById(TableLevel.wantedLevel.ID + 1);
